...
const defaultClassName = cl(border.dottedBottom, table.getCellHeight())
  ...
  NewCol.format = (value, row) => {
    ...
    return React.useCallback(
      ({record, dataModelName, mutateRecords, tdStyle, showHeader}) => {
        ...
        return (
          <Fragment>
            <C_Stack className={cl(`stretching-in-td  justify-start gap-0  leading-[16px]`, stackClassName)}>
              {processedCols.map((item, i) => (
                <R_Stack key={i} style={defaultStyle} className={defaultClassName}>
                  <MemoizedTdContent
                    {...{
                      showHeader,
                      value: item.value,
