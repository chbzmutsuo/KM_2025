...
export type atomTypes = {
  ...
  haishaTableEditorGMF: {user: User; date: Date; tbmDriveSchedule?: any; tbmBase?: any}
  odometerInputGMF: {
    OdometerInput: {
      date: Date
...
export const useJotaiByKey = <S,>(atomKey: atomKey, defaultState: any) => {
  const param: myAtomFamilyParams = {atomKey, defaultState}
  const jotai = useAtom<S>(myAtomFamily(param))
  const [state, setState] = jotai
  const toObj = {state, setState}
  type State = S
  type SetState = (update: State | ((prev: State) => State)) => void

  return [toObj.state, toObj.setState] as [S, SetState]
}
...
export const jotai_GDS_DND = atom<any>({})
