...
const TopPage = () => {
  const {router, session, rootPath, pathname, query} = useGlobal()
  const [navItems, setNavItems] = useState<any[]>([])

  useEffect(() => {
    if (session) {
      const pageGetterMethod = PAGES[`${rootPath}_PAGES`]
      const {navItems} = pageGetterMethod({session, rootPath, pathname, query, dynamicRoutingParams: {}, roles: []})
          ...
          .map(category => (
            <div key={category.label} className="mb-8">
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {category.children?.map((item: pathItemType, i) => {
                  const {tabId, label, ROOT, href} = item
                  ...
                  return (
                    ...
                    >
                      <div className=" flex  items-center justify-center text-xl font-bold text-blue-600">{item.label}</div>
                    </C_Stack>
                  )
                })}
              </div>
