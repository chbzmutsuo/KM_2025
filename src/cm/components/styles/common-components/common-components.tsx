...
export const R_Stack = (props: props) => {
  const {className, ...rest} = props
  return <div className={cl(`row-stack`, className)} {...rest} />
}

export const C_Stack = (props: props) => {
  const {className, ...rest} = props
...
export const CenterScreen = (props: props) => {
...
export const Circle = (props: props & {width?: number; height?: number; color?: colorVariants; inline?: boolean}) => {
  const {className, style: originalStyle, width, height, inline = true, color = 'blue', children, ...rest} = props
  ...
  return (
    <>
      <span className={cl(inline ? 'mx-1 !inline w-fit' : '')}>
        <span
          {...{
            className: cl(`rounded-full border !inline-block `, colorClassMaster.base[color], className),
            style,
            ...rest,
          }}
        >
...
export const Padding = (props: props & {paddingClass?: string}) => {
  const {className, style, ...rest} = props
  return (
    <div
      {...{
...
export const FloatingDataChecker = ({json}) => {
  return (
    <div
      ...
      className={`fixed left-4 top-4  max-h-[300px] max-w-[300px] overflow-auto rounded  bg-white p-2 shadow`}
    >
