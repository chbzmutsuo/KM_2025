...
const MidTableRowGroup = ({
  ...
  const handleToggleAll = () => {
    setlinkedData(prev => {
      if (type === 'linked') {
        return [...Arr.createUpdateDelete(prev, listForGroup).deleteArr]
      } else {
        return [...prev, ...Arr.createUpdateDelete(prev, listForGroup).createArr]
      }
    })
  }
  ...
  const iconClass = cl(
    ` h-5 w-5  cursor-pointer rounded  p-0.5  text-white `,
