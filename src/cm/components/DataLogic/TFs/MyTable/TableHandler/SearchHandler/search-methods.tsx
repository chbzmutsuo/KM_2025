import {formatDate, getMidnight, toUtc} from 'src/cm/class/Days'
    ...
    searchTypes.forEach(type => {
      ...
      keyVals.forEach(keyVal => {
        ...
        if (val.match(/(\d{4})-(\d{2})-(\d{2})/)) {
          ...
          const ModelOnSchema = schema[DH.capitalizeFirstLetter(dataModelName)]
          ...
          val = DH.convertDataType(val, dataType, 'server')
        }
    ...
    Object.keys(SearchColObject)
      ...
      .forEach(key => {
        ...
        if (searchKey && searchValue) {
          DH.makeObjectOriginIfUndefined(searchNotations, searchNotation, [])
