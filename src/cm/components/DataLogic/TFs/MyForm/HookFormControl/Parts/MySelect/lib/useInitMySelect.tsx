...
export default function useInitMySelect(props: VariousInputProps) {
  ...
  // // オプションがセットされていない場合、セットする
  // useEffect(() => {
  //   const optionsUnset = !filteredOptions?.filter(op => op.value)?.length
  //   const validOptionsFound = options?.filter(op => op.value)?.length
  //   if (optionsUnset && validOptionsFound) {
