...
export const updateMultiItemInTransaction = async props => {
  const {MultiItems, latestFormData, initialModelData, updatedModelRes, dataModelName, fetchTransactionAPI} = props
  const {ChildRecordsToUpsert, ChildRecordsToDelete} = (() => {
    ...
    MultiItems.forEach(col => {
      ...
      DH.makeObjectOriginIfUndefined(ChildRecordsToUpsert, model, [])
      DH.makeObjectOriginIfUndefined(ChildRecordsToDelete, model, [])
      ...
      if (i === 0) {
        obj = {
          ...
          format: (value, row) => {
            ...
            return (
              <R_Stack style={{width: 120}} className={`w-full items-center justify-start`}>
                {Children?.map((NthChild, i) => {
                  const Master = NthChild?.[capitalColIdWithoutId]

                  return (
                    <ColorBlock style={{}} key={i} bgColor={Master?.color} className={`p-[1px] text-xs`}>
                      {Master?.name}
                    </ColorBlock>
                  )
                })}
