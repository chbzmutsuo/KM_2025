...
import {Days, formatDate, toUtc} from 'src/cm/class/Days'
...
export const dateSwitcherTemplate = async (props: {query; firstDayOfMonth?: Date; defaultWhere?: any}) => {
  const {query, firstDayOfMonth = Days.getMonthDatum(new Date()).firstDayOfMonth} = props
  ...
  if (FROM && TO) {
    ...
    whereQuery = {gte: FROM, lt: addDays(FROM, 1)}
  } else if (query.month) {
    const MONTH = toUtc(query.month)

    const {firstDayOfMonth, lastDayOfMonth} = Days.getMonthDatum(MONTH)
...
//   const {firstDayOfMonth, lastDayOfMonth, days} = Days.getMonthDatum(today)
