...
export class Grouping {
  ...
  static chart = {
    ...
    prepare_time_curiocity_efficacy_data: AnswerArr => {
      ...
      const data = Arr.sortByKey(sortedAnswer, 'time')
      ...
      const create5MinutesIntervals = () => {
        ...
        while (current <= lastEffectiveData?.time) {
          result.push(current)
          current = addMinutes(current, intervalMinute)
        }
      ...
      intervals.forEach((interval, i) => {
        DH.makeObjectOriginIfUndefined(averagedDataByInterval, String(interval), {
      ...
      effectiveData.forEach((d, i) => {
        ...
        const nextInterval = addMinutes(current, intervalMinute)

        if (time <= nextInterval) {
          DH.makeObjectOriginIfUndefined(averagedDataByInterval, String(nextInterval), {
            average: {efficacy: 0, curiocity: 0, count: 0, efficacyAverage: 0, curiocityAverage: 0},
            list: [],
          })
      ...
      const result = ObjectMap(averagedDataByInterval, (key, value) => {
        ...
        const efficacyAverage = Calc.round(average.efficacy / (average.count || 1))
        const curiocityAverage = Calc.round(average.curiocity / (average.count || 1))
