...
import {getColorStyles, shorten} from '@cm/lib/methods/common'
import {ColorBlock} from '@components/styles/common-components/colors'
              ...
              ...myGame.map((game, index) => {
                ...
                const deleteGame = () => {
                  if (confirm(`本当に削除しますか？`)) {
                    toggleLoad(async () => {
                      await fetchUniversalAPI(`game`, `delete`, {where: {id: game.id}})
                    })
                  }
                }
                  ...
                  <Paper className={`rounded-md`} key={index} style={{width: 320, padding: 10}}>
                    <C_Stack>
                      ...
                      <R_Stack className={`  justify-between`}>
                        <R_Stack>
                          <div className={`w-fit`}>
                            <ColorBlock bgColor={SubjectNameMaster?.color ?? ''}>{SubjectNameMaster?.name}</ColorBlock>
                          </div>
                          <div>第{nthTime}時</div>
