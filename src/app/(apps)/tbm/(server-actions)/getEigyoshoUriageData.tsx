...
export const getEigyoshoUriageData = async ({whereQuery, tbmBaseId}) => {
  ...
  const EigyoshoUriageRecords: EigyoshoUriageRecord[] = userList.map(item => {
    ...
    const red = tail_color.error.main
    const blue = tail_color.blue.main
    ...
    return {
      ...
      keyValue: {
        ...
        C_postalHighway: {
          ...
          style: {fontSize: 12, minWidth: widthBase, color: red},
        },
        D_generalHighway: {
          label: `高速代(一般）`,
          cellValue: MEIAI_SUM(`Q_generalHighwayFee`),
          style: {fontSize: 12, minWidth: widthBase, color: blue},
        },
        E_totalHighway: {
          label: `合計(高速代)`,
