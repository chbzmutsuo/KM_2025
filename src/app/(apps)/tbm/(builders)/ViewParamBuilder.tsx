      ...
      myForm: {
        create: {
          executeUpdate: async props => {
            ...
            const {result: tbmOperation} = await fetchUniversalAPI(`tbmOperationGroup`, 'upsert', {
              where: {id},
              userId,
              tbmBaseId,
              tbmVehicleId,
            ...
            if (go.tbmRouteGroupId) {
              const GoRouteRes = await fetchUniversalAPI(`tbmOperation`, 'upsert', {
                where: {
            ...
            if (back.tbmRouteGroupId) {
              const BackRouteRes = await fetchUniversalAPI(`tbmOperation`, 'upsert', {
                where: {
                  unique_tbmRouteGroup_type: {
