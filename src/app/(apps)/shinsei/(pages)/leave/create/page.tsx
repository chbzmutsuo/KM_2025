        ...
        <BasicForm
          {...{
            ...
            onSubmit: async data => {
              ...
              const {result: me} = await fetchUniversalAPI(`user`, `findUnique`, {
              ...
              // 統括の場合は部長をスキップ
              const nextApproverRole = isTokatsu ? [`統括`] : [`部長`, `統括`]

              let {result: approveUser} = await fetchUniversalAPI(`user`, `findMany`, {
                where: {
                  UserRole: {some: {RoleMaster: {name: {in: nextApproverRole}}}},
              ...
              toggleLoad(async item => {
                const res = await fetchUniversalAPI(`leaveRequest`, `create`, {
                  data: {
                    from,
                    to,
                    leaveType,
