...
export default function PurchaseHistoryTable({dataFetchProps = {}, deletable = false}) {
  ...
  const fetchRequests = async () => {
    try {
      const response = await fetchUniversalAPI(`purchaseRequest`, `findMany`, {
      ...
      const totalResponse = await fetchUniversalAPI(`purchaseRequest`, `findMany`, {
        select: {id: true},
        ...dataFetchProps,
      })
                          ...
                          onClick={async () => {
                            if (confirm(`データを削除しますか?`)) {
                              if (confirm(`本当に削除してもよろしいですか?`)) {
                                await fetchUniversalAPI(`purchaseRequest`, `delete`, {
